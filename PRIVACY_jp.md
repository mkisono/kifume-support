...
### 2.2. 利用情報
- サービス利用履歴（アップロードした動画・XG ファイル、生成した棋譜データ、検索実行履歴など）
- チケット購入・使用履歴
- タグ情報（タグ名、付与対象、作成日時など）
- ログイン履歴
...
4.3. ユーザーがアップロードした動画データは、解析完了後、当サービスが別途定める期間経過後に自動的に削除される場合があります。

4.4. 生成された棋譜データ（メール送付や一時表示のためのデータを含みます）は、作成日から最大7日間保存され、その後自動的に削除される場合があります。

4.5. ユーザーがアップロードした XG ファイルおよび検索用インデックスデータは、原則としてアカウントが有効である限り保持されます。

4.6. タグ情報は、原則として作成したユーザー本人にのみ表示されます。
...
8.2. ユーザーがアップロードした動画データは、解析完了後、当サービスが別途定める期間経過後に自動的に削除される場合があります。

8.3. 生成された棋譜データ（メール送付や一時表示のためのデータを含みます）は、作成日から最大7日間保存され、その後自動的に削除される場合があります。

8.4. ユーザーがアップロードした XG ファイルおよび検索用インデックスデータは、原則としてアカウントが有効である限り保持されます。ユーザーによる削除操作、アカウント削除、または法令等に基づく必要がある場合には削除または保持制限を行うことがあります。

8.5. サービス利用履歴やログ情報は、セキュリティ管理、不正利用防止、法的義務履行のために、最長3年間保持される場合があります。
