...
当サービスは、バックギャモン関連データを解析・管理するオンラインサービスです。ユーザーは動画ファイルまたは XG ファイルをアップロードし、棋譜データの生成・検索機能を利用できます。なお、チケットが必要な機能（例: 棋譜データのメール受け取り、「Search」、「XG Upload」など）では、所定の条件に従ってチケットを利用または消費します。
...
4.1. 当サービスでは、当サービスが指定する一部機能の利用に「チケット」が必要です。

4.2. チケットは当サービスのウェブサイトで購入することができます。チケットの価格は予告なく変更される場合があります。

4.3. チケットの有効期限は購入日から180日間です。期限を過ぎたチケットは自動的に失効し、返金や延長はできません。

4.4. ユーザーは、チケットが必要な機能の利用時に、当サービスが定める方法でチケットを利用または消費します。必要なチケット数は、解析されたゲーム数、処理対象データ量、または当サービスが別途定める基準によって異なります。
...
6.1. ユーザーがアップロードまたは入力する動画、画像、XG ファイル、棋譜データ、タグ、検索条件、その他関連メタデータ等のコンテンツ（以下「ユーザーコンテンツ」といいます）の所有権は、ユーザーに帰属します。

6.2. ユーザーは、当サービスに対し、ユーザーコンテンツを、サービス提供、運用、保守、改善、セキュリティ確保、およびユーザーが共有または公開を許可した範囲での検索提供のために必要な範囲で利用、複製、改変、表示するための非独占的かつ無償のライセンスを許諾するものとします。
...
7.2. ユーザーがアップロードした動画データは、解析完了後、当サービスが別途定める期間経過後に自動的に削除される場合があります。

7.3. 生成された棋譜データ（メール送付や一時表示のためのデータを含みます）は、作成日から一定期間（最大7日間）保存され、その後自動的に削除される場合があります。

7.4. ユーザーがアップロードした XG ファイルおよび検索用インデックスデータは、原則としてアカウントが有効である限り保持されます。ユーザーによる削除操作、アカウント削除、または法令等に基づく必要がある場合には削除または保持制限を行うことがあります。

7.5. タグ情報は原則として付与したユーザー本人にのみ表示されます。
